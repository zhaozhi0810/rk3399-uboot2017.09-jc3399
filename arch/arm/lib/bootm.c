...
{
#ifndef CONFIG_TARGET_JC_RK3399	
...
//#ifdef CONFIG_TARGET_JC_RK3399
	...
	char lcd_bootargs[64] = {0};

	sprintf(new_bootargs,"%s ubootversion=\"%s %s %s\"",old_bootargs,U_BOOT_VERSION,U_BOOT_DMI_DATE,U_BOOT_TIME);
	if(gd->lcd_type>=0)
	{
		sprintf(lcd_bootargs," lcd_type=%d ",gd->lcd_type);
		strcat(new_bootargs,lcd_bootargs);	  
	}
