...
static void init_display_buffer(ulong base)
{
	...
	//printf("2024-08-28 memory_start = %lu,DRM_ROCKCHIP_FB_SIZE=%d\n",memory_start,DRM_ROCKCHIP_FB_SIZE);
	//memset((void*)memory_start,0,DRM_ROCKCHIP_FB_SIZE);
}
...
void *get_display_buffer(int size)
{
	...
	//printf("2024-08-28 buf = %p,size=%d\n",buf,size);
...
#ifdef CONFIG_TARGET_JC_RK3399      //2023-09-15
		printf("gd->lcd_type = %d\n",gd->lcd_type);
		if(gd->lcd_type >= 0)
		{
			if(gd->lcd_type == 2)	//捷诚第二块5寸屏			
...
static int rockchip_display_probe(struct udevice *dev)
{
	...
	ofnode_for_each_subnode(node, route_node) {
		...
		ret = ofnode_read_string_index(node, gd->lcd_type>=0?"logo,uboot":"logo_r,uboot", 0, &name);
		if (!ret)
			memcpy(s->ulogo_name, name, strlen(name));
		ret = ofnode_read_string_index(node, gd->lcd_type>=0?"logo,kernel":"logo_r,kernel", 0, &name);
		if (!ret)
