...
#ifdef CONFIG_TARGET_JC_RK3399     //2023-09-15 增加一个选择项
//返回-1 就是超时，-3表示校验和错误。
static int com4_recv_lcd_type_data(void)
{
	char cmd_buf[12];
	int count = 0;
	int ret_val = 0;
...
//返回-1 就是超时，-3表示校验和错误。
//只对话音接口板生效！！！！
static int com0_recv_mcu_version_data(void)
{
	char cmd_buf[12];
	int count = 0;
	int ret_val = 0;
//	int val;
	...
	//	printf("ret_val = %x\n",ret_val);
		...
		else if(count >= 3)   //接收结束
		{
			printf("com0 mcu_version data :%x %x %x %x\n",cmd_buf[0],cmd_buf[1],cmd_buf[2],cmd_buf[3]);
					
			if(((cmd_buf[0]+cmd_buf[1]+cmd_buf[2])&0xff) ==  cmd_buf[3])
				return cmd_buf[2];
...
static int rockchip_panel_ofdata_to_platdata(struct udevice *dev)
{
	...
	int ret;
/************************************************************************************************/
#ifdef CONFIG_TARGET_JC_RK3399  //zhaozhi added 2023-03-14
	static int lcd_type;
	static int mcu_initd = 0;  //不需要反复初始化，2023-07-31
	struct udevice *iicbus;
	struct udevice *iicdevp;

	if(mcu_initd == 0)  //不需要反复初始化，2023-07-31
	{
		com4_init();
		com0_init();   //用于接口板
		//先发送一次试试。
		com4_send_cmd();
		com0_send_check_version_cmd();   //一起发了
		gd->lcd_type = -1;
	//	mdelay(5);
		ret = com4_recv_lcd_type_data();
		if(ret < 0) //没有应答就等2秒
		{		
			if(com0_recv_mcu_version_data()>0) //话音接口板有应答
			{
			//	ret = -1;			
	   ...
	   //导光面板初始化iic3，有触摸屏（0x41，0x2a,0x5c）和键盘0x40
		...
		mcu_initd = 1;  //表示已经初始化了，不再需要初始化了。2023-07-31
	}
#endif
